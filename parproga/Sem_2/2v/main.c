...
int main(int argc, char **argv)
{
    ...
    // выдаем блок каждому исполнителю
    ...
    int* recvcnts = calloc(commsize, sizeof(int)); //массив размеров блоков
    handleCallocError(recvcnts);

    int* displs = calloc(commsize, sizeof(int)); //массив смещений блоков относительно начала строки
    handleCallocError(displs);
    
    for (int k = 0; k < commsize; ++k) //собираем со всех локальные размеры блоков
    {
    ...
    displs[0] = 0;
    for (int i = 1; i < commsize; ++i) //считаем смещения
    {
